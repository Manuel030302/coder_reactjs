...
export const getUnJuego = (id) =>{
    return new Promise (( resolve, reject ) => {
        setTimeout(() =>{
            const juego = catalogo.find(game => game.id == id);
            resolve(juego);
        }, 2000)
    })
}

export const getJuegoPorCategoria = (idCategoria) =>{
    return new Promise (( resolve, reject ) => {
        setTimeout(() =>{
            const juegoCategoria = catalogo.filter(game => game.idCat == idCategoria)
            resolve(juegoCategoria);
        }, 2000)
    })
}